...
class UnirestTransactionLog(TransactionLogBase):
    ...
    def dispatch(self, method, url, params={}, headers=None, *a, **kw):
        parsed_url = urlparse(url)
        request_headers, request_payload, transaction_id, method_code = self.before(parsed_url, params, headers)
    ...
    def before(self, parsed_url, request_params, request_headers):
    ...
    def after(
            self, method, parsed_url, request_time, response_time, request_headers, request_payload, response,
            transaction_id, method_code
    ):
